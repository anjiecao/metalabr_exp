...
all_slope_estimates %>% saveRDS(here("cached_data/all_slope_estimates.Rds"))
...
```{r}
full_age_models_df <- lapply(unique(full_d$ds_clean), 
       function(name){
         print(name)
         get_compare_IC_df(full_d, name, NULL)
       }) %>% 
  bind_rows()
...
categorized_age_pred_df <- all_age_pred %>% 
  ...
  mutate(
    ds_type = case_when(
      ds_name %in% c( "Language discrimination and preference", 
                     ...
                     "Statistical word segmentation", "Audio-Visual Congruence", 
                     "Language discrimination and preference (discrimination)", 
                     "Language discrimination and preference (preference)") ~ "Sounds", 
  ...
  select(ds_name, ds_type, mean_age_months, pred, ci.ub, ci.lb, model_type) %>% 
  ungroup()

significant_res = all_slope_estimates %>% 
...
categorized_d <- d %>% 
  mutate(
    ds_type = case_when(
      ds_clean %in% c( "Language discrimination and preference", 
                     "Mispronunciation sensitivity", 
                     "Natural speech preference", "Sound symbolism", "Statistical sound category learning", 
                     "Vowel discrimination (native)",
                     "Vowel discrimination (non-native)" ,"Word Segmentation (combined)",
      ...
      ds_clean %in% c("Familiar word recognition", 
                     "Cross-situational word learning", 
                      "Label advantage in concept learning",
                     "Mutual exclusivity",
                     "Online word recognition",
      ...
      ds_clean %in% c("Gaze following (combined)",
                     "Neonatal Imitation",
                     "Prosocial agents",
                     #"Symbolic play", 
                     "Infant directed speech preference") ~ "Communication", 
      
      ds_clean %in% c("Abstract rule learning",  
                     "Categorization bias", 
                     "Simple arithmetic competences") ~ "Cognitive"
     )
  ) %>% 
...
categorized_d <- categorized_d %>% 
  rename(ds_name = ds_clean) %>% 
  mutate(
     ds_name = case_when(
      ds_name == "Cross-situational word learning" ~ "X-situation learning", 
      ds_name == "Label advantage in concept learning" ~ "Label advantage",
      ds_name == "Language discrimination and preference (discrimination)" ~ "Language discrimination",       ds_name == "Audio-Visual Congruence" ~ "Audio-visual congruen.", 
  ...
  #mutate(
  #  ds_name = case_when(
  #    ds_name %in% significant_res ~ paste0(ds_name, "*"), 
  #    TRUE ~ ds_name
  #  )
  #)
```
...
```{r}
full_age_df_wide <- full_age_models_df %>% 
  filter(ic == "AICc") %>% 
  rename(Dataset = dataset) %>% 
  select(REML, model_spec_clean, Dataset) %>% 
  pivot_wider(names_from = model_spec_clean, 
...
full_age_df_wide$min <- apply(full_age_df_wide[c('Linear','Log', 'Quadratic', 'Const')], 1, min)

# get the delta value 
full_age_df_wide <- full_age_df_wide %>% 
  mutate(d_linear = Linear - min, 
         d_log = Log - min, 
         d_quad = Quadratic - min, 
         d_const= Const - min) %>% 
...
full_age_df_wide %>% 
  pivot_longer(cols = c("Linear", "Log", "Quadratic", "Const")) %>% 
  mutate(type = "Full age range") %>% 
  bind_rows(
    age_df_wide %>% 
    pivot_longer(cols = c("Linear", "Log", "Quadratic", "Const")) %>% 
    mutate(type = "36MO") 
  ) %>% 
  ggplot(aes(x = reorder(Dataset, value), y = value, color = name, shape = type, group = type)) +
  theme_few() + 
  geom_hline(yintercept = 4, linetype = 2, color = "gray") + 
  geom_hline(yintercept = 0, linetype = 3) + 
...
age_models_df %>% 
  ...
  select(Dataset, log, quad, linear) %>% 
  filter(log > 4 | quad > 4 | linear > 4)
```
...
```{r}
age_diff_df <- d %>% 
  select(ds_clean, mean_age_months) %>% 
  group_by(ds_clean) %>% 
  arrange(mean_age_months,.by_group = TRUE) %>% 
  mutate(age_diff = mean_age_months - lag(mean_age_months)) %>% 
  filter(!is.na(age_diff)) %>% 
  group_by(ds_clean) %>% 
  tidyboot::tidyboot_mean(age_diff)
```
```{r}
age_diff_df %>% 
  ggplot(aes(x = reorder(ds_clean, -mean), y = mean, ymin = ci_lower, ymax = ci_upper)) + 
  geom_pointrange() + 
  coord_flip() + 
  theme_few()
```
...
## current try out w/o label 
```{r}


label_df <- categorized_age_pred_df %>% 
  group_by(ds_name) %>% 
  filter(mean_age_months == max(mean_age_months), model_type == "linear") %>%
  distinct(ds_name, mean_age_months, pred, ds_type) 

#categorized_age_pred_df %>% filter(ds_type == "Words") %>% distinct(ds_name)
  

label_df$x = label_df$mean_age_months 
label_df$y = label_df$pred 

label_df <- label_df %>% 
  mutate(
    x = case_when(
      ds_name == "Categorization bias" ~ x + 1, 
      ds_name == "Neonatal Imitation" ~ x + 1,
      ds_name == "Prosocial agents" ~ x + 1,
      ds_name == "Mutual exclusivity" ~ x - 3, 
      ds_name == "Cross-situational word learning" ~ x -3,
      ds_name == "Label advantage in concept learning" ~ x - 10, 
      ds_name == "Syntactic bootstrapping" ~ x-1,
      ds_name == "Vowel discrimination (native)" ~ x + 1.5,
      ds_name == "Statistical word segmentation"  ~ x + 1,
      ds_name == "Audio-Visual Congruence" ~ x - 7, 
       ds_name == "Language discrimination and preference (preference)" ~ x + 5,
      ds_name == "Natural speech preference" ~ x + 6,
       ds_name == "Statistical sound category learning" ~ x +6,
      ds_name == "Vowel discrimination (native)" ~ x - .8,
      ds_name == "Vowel discrimination (non-native)" ~ x - 10,
      ds_name == "Sound symbolism" ~ x -.4,
    ...
    y = case_when(
      ds_name == "Categorization bias" ~ y + 2, 
      ds_name == "Simple arithmetic competences" ~ y + .5,
      ds_name == "Abstract rule learning" ~ y - .5,
      ds_name == "Gaze following (combined)" ~ y + .5,
      ds_name == "Prosocial agents" ~ y + .5,
      ds_name == "Infant directed speech preference" ~ y - .5,
      ds_name == "Cross-situational word learning" ~ y + .5,
      ds_name == "Familiar word recognition" ~ y + .5,
      ds_name == "Label advantage in concept learning" ~ y - .8, 
      ds_name == "Syntactic bootstrapping" ~ y-.5,
       ds_name == "Switch task" ~ y + .3,
      ds_name == "Mispronunciation sensitivity" ~ y + .5,
      ds_name == "Vowel discrimination (native)" ~ y - .45,
       ds_name == "Statistical word segmentation"  ~ y - .5,
      ds_name == "Statistical sound category learning" ~ y+.1,
      ds_name == "Natural speech preference" ~ y + .8,
      ds_name == "Audio-Visual Congruence" ~ y + .8,
      ds_name == "Word Segmentation (combined)" ~ y - .4, 
      ds_name == "Language discrimination and preference (preference)" ~ y + .8,
       ds_name == "Vowel discrimination (non-native)" ~ y + .8,
       ds_name == "Sound symbolism" ~ y + .3,
      TRUE ~ y
    ), 
    ...
    label = case_when(
      ...
      ds_name == "Gaze following (combined)" ~ "Gaze following", 
      ds_name == "Label advantage in concept learning" ~ "Label advantage", 
      ds_name == "Statistical sound category learning" ~ "Stats. category learning",
      ds_name == "Statistical word segmentation" ~ "Stats. word seg.",
      ds_name == "Word Segmentation (combined)" ~ "Word segmentation", 
      ds_name == "Infant directed speech preference" ~ "IDS pref.",
      TRUE ~ ds_name
    )
  ) #%>% 
  #filter(label == "Vowel discrimination (native)")
...
categorized_age_pred_df %>% 
  ...
  mutate(
    label =  if_else(mean_age_months == max(mean_age_months) & model_type == "linear", as.character(ds_name), NA_character_)
  ) %>% 
  ungroup() %>% 
 #filter(label == "Switch task") %>% 
  ggplot(aes(x = mean_age_months,  y = pred,  color = ds_name)) + 
  geom_hline(yintercept = 0, linetype ="dashed", alpha = .5) + 
  geom_smooth(data = categorized_d, #%>% filter(ds_clean == "Vowel discrimination (native)"), 
              aes(x = mean_age_months, y = d_calc, group = ds_clean, color = ds_clean), 
              method = "loess", span = 2,  se = FALSE)+ 
  geom_line(alpha = .5, linetype = "dashed", aes(group = group_var)) + 
   

 geom_ribbon(
              aes(x = mean_age_months, group = group_var,y = pred, ymin = ci.lb, ymax = ci.ub, fill = ds_name), alpha = 0.05, colour = NA)+ 
...
xlab("Mean age (months)") + 
  ...
  facet_wrap(~ds_type, scales = "free_y")  
...
label_df %>% 
  mutate(
    x = case_when(
      ds_name == "Categorization bias" ~ x + 5, 
      TRUE ~ x
    ), 
    
    y = case_when(
      ds_name == "Categorization bias" ~ y + 1, 
      TRUE ~ y
    )
...
get_all_age_model_prediction <- function(ds_name, min_age_df, formula, model_type, all_ds){
