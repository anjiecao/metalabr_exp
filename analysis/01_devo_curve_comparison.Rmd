...
categorized_age_pred_df %>% 
  ggplot(aes(x = mean_age_months, y = pred, group = model_type, color = ds_name)) + 
...
categorized_age_pred_df %>% 
  ...
  ggplot(aes(x = mean_age_months, y = pred, group = group_var, color = ds_name)) + 
  geom_hline(yintercept = 0, linetype ="dashed", alpha = .5) + 
  geom_smooth(data = categorized_d, 
  ...
  geom_ribbon(
              aes(x = mean_age_months, y = pred, ymin = ci.lb, ymax = ci.ub, fill = ds_name), alpha = 0.2, colour = NA,)+
    theme_few()+
